...
export const mapDataToProgram = (program, programDays, userName) => {
  ...
  const dailyPrograms = generateDailysPrograms(
    program,
    programDays,
    programObj
  );
...
const initializeProgramObject = (program, programDays, userName) => {
  return {
    userName: userName,
    type: program.type,
    minRate: program.targets["יעד מינימלי לאימון"]
      ? program.targets["יעד מינימלי לאימון"]
      : 0.8,
    startDate: moment(),
    ...
    const subItems = item.subItems.map((subItem) => {
      return programType == "training"
        ? {
            ...
            columnsValues: [subItem.replications, subItem.weight],
          }
        : {
            name: subItem.label,
            done: false,
            columnsValues: [subItem.amount, subItem.unit, 0],
          };
    });
