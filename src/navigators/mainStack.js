...
export const mainStack = (user, programs) => {
	const headerTitle = (navigation) => {
		return (
			<TouchableOpacity
				...
				onPress={user.programs ? () => navigation.navigate("ראשי") : null}
			>
				<Text
					style={{
						fontSize: 20,
				...
				<Image source={require("../../assets/adaptive-icon.png")} style={{ height: 50, width: 50 }} />
			</TouchableOpacity>
		);
	};
	...
	return (
		<Stack.Navigator>
			...
			<Stack.Screen name="programTargets" component={ProgramTargetsScreen} options={options("בנה תכנית", user)} />
			<Stack.Screen name="programItems" component={CreateItemsScreen} options={options("בנה תכנית", user)} />
			<Stack.Screen
				name="programSummary"
